"""
2d steady-state ADR equation on a UK grid
...
if __name__ == '__main__':
    ...
    if USER_INPUT:
        ...
        plot_uk_mesh(nodes, dirichlet_boundary, IEN, lw, show=True)
    ...
    lap, stn, force, _, u, mask = solve(cg, lm, NO_OF_ELEMS, P1, P2, VEL_FIELD, KAPPA)
    # print(f"Solutions:\n{u}")
    # print(f"Global Laplacian matrix:\n{lhs}")
    # print(f"Global force vector:\n{rhs}")

    """ Solve linear system """
    u[mask] = sparse.linalg.spsolve(lap + stn, force)
    u /= np.max(u)
